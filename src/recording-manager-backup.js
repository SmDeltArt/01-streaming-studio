...
export default class RecordingManager {
    constructor(app) {
        ...
        this.recordedChunks = [];
        this.iframeRecordingMode = false;
    ...
    resetRecordingState() {
        ...
        this.recordingStartTime = null;
        this.pausedDuration = 0;
        this.iframeRecordingMode = false; // Reset iframe recording mode
    ...
    showRecordingComplete(recordingUrl, filename, blob) {
        // Check if this was an iframe recording
        if (this.iframeRecordingMode && window.enhancedIframeRecording) {
            try {
                const iframeDimensions = window.enhancedIframeRecording.setupIframeRecording();
                window.enhancedIframeRecording.createCropNotification(blob, filename, iframeDimensions);
                return;
            } catch (error) {
                console.warn('Iframe recording crop failed, showing standard notification:', error);
            }
        }
    ...
    async setupIframeRecording() {
        try {
            // Get iframe element and its dimensions
            const iframe = document.querySelector('#contentFrame');
            if (!iframe || iframe.style.display === 'none') {
                throw new Error('No visible iframe found. Please load content first.');
            }

            const iframeRect = iframe.getBoundingClientRect();
            const canvasWidth = Math.floor(iframeRect.width);
            const canvasHeight = Math.floor(iframeRect.height);

            // Create hidden canvas for iframe content capture
            this.iframeCanvas = document.createElement('canvas');
            this.iframeCanvas.width = canvasWidth;
            this.iframeCanvas.height = canvasHeight;
            this.iframeCanvas.style.position = 'absolute';
            this.iframeCanvas.style.left = '-9999px';
            this.iframeCanvas.style.top = '-9999px';
            document.body.appendChild(this.iframeCanvas);

            this.iframeContext = this.iframeCanvas.getContext('2d');
            this.iframeRecordingMode = true;

            console.log(`Iframe recording setup: ${canvasWidth}x${canvasHeight}`);
            return this.iframeCanvas.captureStream(30); // 30 FPS
        } catch (error) {
            console.error('Error setting up iframe recording:', error);
            throw error;
        }
    }

    startIframeContentCapture() {
        if (!this.iframeRecordingMode || !this.iframeCanvas || !this.iframeContext) {
            return;
        }
        ...
        const captureFrame = () => {
            if (!this.isRecording || !this.iframeRecordingMode) {
                return;
            }
            ...
            try {
                ...
                canvas.width = this.iframeCanvas.width;
                canvas.height = this.iframeCanvas.height;

                // Clear the iframe canvas
                this.iframeContext.clearRect(0, 0, this.iframeCanvas.width, this.iframeCanvas.height);

                // Try to capture iframe content if accessible
                try {
                    if (iframe.contentDocument) {
                        // Same-origin iframe - can access content
                        const iframeDoc = iframe.contentDocument;
                        const iframeBody = iframeDoc.body || iframeDoc.documentElement;
                        
                        // Create image from iframe content (simplified approach)
                        this.iframeContext.fillStyle = '#ffffff';
                        this.iframeContext.fillRect(0, 0, this.iframeCanvas.width, this.iframeCanvas.height);
                        
                        // Add text indicator for iframe content
                        this.iframeContext.fillStyle = '#333333';
                        this.iframeContext.font = '16px Arial';
                        this.iframeContext.textAlign = 'center';
                        this.iframeContext.fillText(
                            'Iframe Content Recording', 
                            this.iframeCanvas.width / 2, 
                            this.iframeCanvas.height / 2
                        );
                    } else {
                        // Cross-origin iframe - show placeholder
                        this.iframeContext.fillStyle = '#f0f0f0';
                        this.iframeContext.fillRect(0, 0, this.iframeCanvas.width, this.iframeCanvas.height);
                        
                        this.iframeContext.fillStyle = '#666666';
                        this.iframeContext.font = '14px Arial';
                        this.iframeContext.textAlign = 'center';
                        this.iframeContext.fillText(
                            'Iframe Content Area', 
                            this.iframeCanvas.width / 2, 
                            this.iframeCanvas.height / 2 - 10
                        );
                        this.iframeContext.fillText(
                            `${this.iframeCanvas.width}x${this.iframeCanvas.height}`, 
                            this.iframeCanvas.width / 2, 
                            this.iframeCanvas.height / 2 + 10
                        );
                    }
                    ...
                    this.iframeContext.fillStyle = '#e8f4f8';
                    this.iframeContext.fillRect(0, 0, this.iframeCanvas.width, this.iframeCanvas.height);
                    
                    this.iframeContext.strokeStyle = '#4a90a4';
                    this.iframeContext.setLineDash([5, 5]);
                    this.iframeContext.strokeRect(2, 2, this.iframeCanvas.width - 4, this.iframeCanvas.height - 4);
                    
                    this.iframeContext.fillStyle = '#2c5aa0';
                    this.iframeContext.font = 'bold 16px Arial';
                    this.iframeContext.textAlign = 'center';
                    this.iframeContext.fillText(
                        'Recording Iframe Area', 
                        this.iframeCanvas.width / 2, 
                        this.iframeCanvas.height / 2 - 10
                    );
                    this.iframeContext.font = '12px Arial';
                    this.iframeContext.fillText(
                        `${this.iframeCanvas.width}x${this.iframeCanvas.height} pixels`, 
                        this.iframeCanvas.width / 2, 
                        this.iframeCanvas.height / 2 + 10
                    );
                }
    ...
    cleanupIframeRecording() {
        this.iframeRecordingMode = false;
        if (this.iframeCanvas) {
            document.body.removeChild(this.iframeCanvas);
            this.iframeCanvas = null;
        }
        this.iframeContext = null;
    }
    ...
    async startRecording(options = {}) {
        try {
            ...
            // Handle iframe recording mode
            if (source === 'iframe') {
                console.log('Starting iframe-specific recording...');
                this.iframeRecordingMode = true;
                
                // First try to get screen capture
                try {
                    ...
                    // Setup iframe canvas recording
                    const iframeStream = await this.setupIframeRecording();
                    
                    // Use iframe stream as primary with screen audio
                    captureStream = iframeStream;
                    ...
                    // Start iframe content capture
                    this.startIframeContentCapture();

                    // Stop the screen video track (we only needed audio)
                    screenStream.getVideoTracks().forEach(track => track.stop());

                } catch (screenError) {
                    console.warn('Screen capture failed, using iframe canvas only:', screenError);
                    captureStream = await this.setupIframeRecording();
                    this.startIframeContentCapture();
                }
            } else {
            ...
            if (!captureStream) {
                throw new Error('Failed to obtain capture stream');
            }
            ...
            this.updateRecordingStatus('recording', source === 'iframe' ? 'Recording iframe...' : 'Recording...');
            this.app.recordBtn.setAttribute('data-recording', 'true');
            this.app.recordBtn.querySelector('.label').innerHTML = 'Stop Recording <span class="shortcut">(R)</span>';
            this.app.recordBtn.querySelector('.icon').textContent = '⏹️';
            ...
            const recordingType = source === 'iframe' ? 'Iframe recording' : 'Screen recording';
            console.log(`${recordingType} started successfully`);
            
        } catch (error) {
            console.error('Error starting recording:', error);
            this.showError('Recording Error', error.message || 'Failed to start recording. Please check permissions.');
            this.updateRecordingStatus('error', 'Recording failed');
            this.cleanupIframeRecording();
        }
    }
